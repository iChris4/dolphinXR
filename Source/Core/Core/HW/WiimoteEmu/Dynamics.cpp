                  ...
                  const ControllerEmu::InputOverrideFunction& override_func, float time_elapsed)
{
  ...
  state->position = {0, distance, -height};
  state->velocity = {};
  state->acceleration = {};
  ...
  if (override_func)
  {
    // When an input override is active (e.g. OpenXR VR controllers), recentering resets
    // the override's reference orientation via a "Recenter" signal. Skip the offset-based
    // recenter to avoid shifting the usable cursor range off-screen.
    if (recenter_pressed && !state->recenter_button_held)
      override_func(ir_group->name, "Recenter", 0.0);
    state->recenter_button_held = recenter_pressed;

    centered_x = std::clamp(cursor.x, -1.0, 1.0);
    centered_y = std::clamp(cursor.y, -1.0, 1.0);
    state->recenter_x = 0.0f;
    state->recenter_y = 0.0f;
  }
  else
  {
    if (recenter_pressed && !state->recenter_button_held)
    {
      state->recenter_x = cursor.x;
      state->recenter_y = cursor.y;
    }
    state->recenter_button_held = recenter_pressed;

    centered_x = std::clamp(cursor.x - state->recenter_x, -1.0, 1.0);
    centered_y = std::clamp(cursor.y - state->recenter_y, -1.0, 1.0);
  }

  const auto target_angle = Common::Vec3(pitch_scale * -centered_y, 0, yaw_scale * -centered_x);
