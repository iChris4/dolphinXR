      ...
      m_last_update(Clock::now())
{
  ...
  AddSetting(&m_sensitivity_setting,
             {_trans("Pointer Sensitivity"),
              // i18n: The percent symbol.
              _trans("%"),
              _trans("Scales pointer movement. Lower values require larger controller movement for "
                                   ...
                                   const ControllerEmu::InputOverrideFunction& override_func)
{
  ...
  const ControlState sensitivity_scale = m_sensitivity_setting.GetValue() / ControlState(100.0);

  if (!override_func)
  {
    if (state.IsVisible())
    {
      state.x = std::clamp(state.x * sensitivity_scale, ControlState(-1.0), ControlState(1.0));
      state.y = std::clamp(state.y * sensitivity_scale, ControlState(-1.0), ControlState(1.0));
      if (m_invert_x_setting.GetValue())
        state.x = -state.x;
    }
    return state;
  }
